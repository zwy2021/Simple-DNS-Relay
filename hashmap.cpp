...
//collide
HashNode* lookup(char* s, struct HashNode* map[], int size) {
	unsigned index = hash(s,size);
	HashNode *n = map[hash(s,size)];
	for (; n != NULL; n = n->next) {
		if (!strcmp(n->key, s)) {
			return n;
		}
...
char* get(char* key, struct HashNode* map[], int size) {
	HashNode* hn = lookup(key,map,size);
	if (hn == NULL) {
		return NULL;
...
void deleteMapNode(const char domain[], struct HashNode* map[], int size) {
	int index=hash(const_cast<char*>(domain),size);
...
bool findNode(unsigned char ipAddr[4], const char domain[], struct HashNode* map[],int size) {
	char *tmp = get(const_cast<char*>(domain), map,size);
	if (tmp == NULL) {
		return false;
	}
	char byte[3];
