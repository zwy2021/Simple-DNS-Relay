...
{
	char *tmp = get(const_cast<char*>(domain), cachemap, CACHESIZE);
	if (tmp) {
		...
		if (curSize < CACHESIZE) {
			install(const_cast<char*>(domain), (char*)ipAddr, cachemap, CACHESIZE);
			if (curSize == 0) {
				Node * tmp = head;
				head = q;
				tail = q;
			...
			tail = tail->pre;
			tail->next = NULL;
			deleteMapNode(const_cast<char*>(q->domain), cachemap, CACHESIZE);
			free(q);
		}
